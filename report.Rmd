...
Let us describe the data given a little bit and take a peek at it values and ranges:
...
So about $12\%$ of the instances in `feature_2` are `NA` so they should not be ignored. It is interesting to see the number of instances for each group in `feature_1` and also the distribution of `NA` in said groups.
...
Another interesting one is `feature_3` which seems drawn from a theoretical distribution, we will explore deeper there later on. This `feature_3` is highly correlated to `feature_2` and `feature_11`.`feature_14` seems to be correlated to `feature_1`, and outside these examples the other features do not have a relevant correlation among them. `feature_8` presents a behaviour that also seems drawn from the combination of theoretical probability distributions.
...
ggpairs(weekly.df.grouped, 
		columns = input$feat,  
		aes(colour = weekly.df.grouped$feature_18, 
			alpha = 0.5))+
		theme_minimal()+
...
We can see how the couriers behave differently whether they churn or not in `feature_3` and the other features correlated to it. Now depending on the meaning of the variables one could infer in what makes a courier work more or less than the others. In the other features we do not see much difference between the couriers, so it may be that the couriers behave similarly overall except for the features mentioned before, which could give us a hint on why do they only have worked one week.
...
The most relevant feature for the model is `feature_3`. Next we have `feature_11` and `feature_2` which is no surprise given that they are highly correlated to `feature_3`. Then we have `worked_weeks`. We want to dissert for a bit here to talk about why we did not fill the missing weeks for the couriers. It seems that the courier missing weeks of work is a relevant feature for distringuishing the label and it has something to do with the behaviour of the courier. As we can see in the plot of correlations between features, `worked_weeks` is correlated with relevant features like `feature_3`. We could have filled the missing week instances for couriers using bootsrap for instance to preserve the distribution of the features. But it does not seem apropriate given that the couriers express different behaviours depending of how many weeks they worked, as we see in the `worked_weeks` feature and also in the churn plots. So in the end we would be doing either two things: filling the `NA` with statistically representative data without still being able to distinguish them because we do not take into account the different behaviour that the couriers exhibit, or we could take this into account and introduce bias in our model, as we can only represent their behaviour with the data we are provided, which is not necessarely the real one.

Here now we show the performance of the model:
...
With this model we have a higher `OOB` error of about $25\%$ so it seems that overall the model is worse. But if we look closely at the classification errors we can see that the classification error for the `0` has reduced from $74\%$ to $22\%$, while the error for `1` has increased from $3\%$ to $30\%$. This does not mean that the model is necessarely worse, it depends on what the porpuse of the model is. For instance imagine that we want to classify patients from a hospital in the categories `ill` and `not ill`. For a doctor it is crucial to know with high accuracy if the patient is ill and will not tolerate high error in this classification, but will not mind if a healthy patient is classified as `ill`. In this case the first model is useful if the `1` are ill patients, as it has low classification error. Another situation could be that we need to predict the credit default for clients in a bank. It would happen that we have low data for the ones defaulting the credit, as in the case of our `0`. A model that does not classify them well is of no interest to the bank, even if we are able to classify very well the non default clients. For the bank it is more useful a model like the second one, where the overall classification error is higher, but the label of interest has lower misclasification. In our case, we do not know which is the intention of this classification as we do not know exactly what the label means, or the variables involved. We present both models that could be suited for both cases.